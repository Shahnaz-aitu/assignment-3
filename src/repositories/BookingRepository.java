...
public class BookingRepository implements IBookingRepository {
    ...
    public List<BookingDetails> getUserBookings(int userId) {
        ...
        String sql = "SELECT b.id, b.room_id, b.check_in_date, b.check_out_date, " +
                "COALESCE(r.type, 'Unknown') AS room_type, r.price AS room_price, h.name AS hotel_name " +
                "FROM bookings b " +
    ...
    // Новый метод для получения полной информации о бронировании
    public BookingDetails getFullBookingDescription(int bookingId) {
        ...
        String sql = "SELECT b.id AS booking_id, b.check_in_date, b.check_out_date, " +
                "b.status, u.id AS user_id, u.name AS user_name, u.email AS user_email, COALESCE(u.age, 18) AS user_age, " +
        ...
        try (Connection con = db.getConnection(); PreparedStatement stmt = con.prepareStatement(sql)) {
            ...
            if (rs.getInt("user_id") != 0) {
                if (age < 18) {
                    ...
                    age = 18; // Автоматически исправляем возраст
                }
                user = new User(
                        rs.getInt("user_id"),
